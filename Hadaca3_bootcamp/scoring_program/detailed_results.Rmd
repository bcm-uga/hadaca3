...
```{r label = "reading datasets and groundtruth" }
...
for (groundthruth_name in groundtruh_list){
  ...
  dataset_name = paste0("mixes_",methods_name,".h5")

  dic_longnames2short[[dataset_name]] = methods_name


  gt_file =  read_hdf5(paste0(dir_name,.Platform$file.sep, groundthruth_name) )$groundtruth
  ...
  score_data = read_hdf5(score_file )
  score[[methods_name ]] = score_data
}

prediction <- read_hdf5(paste0(input,.Platform$file.sep,"res",.Platform$file.sep,"prediction.h5"))
time = read_hdf5(paste0(input, .Platform$file.sep, "res", .Platform$file.sep, "Rprof.h5") )
...
for (data in datasets) {
  ...
  metrics <- c(
  "rmse","mae","aitchison",
  "pearson_tot","pearson_col","pearson_row",
  "spearman_tot","spearman_col","spearman_row"
  )

  score_numeric <- score_data[metrics]
