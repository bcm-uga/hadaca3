...
baseline_scores = baseline_scoring_function(A_real=A_real, A_pred=Aest, time=timing)
# rownames(baseline_scores$baseline_estimation) = baseline

saveRDS(baseline_scores, paste0(output,"/scores.rds"))
...
cat(paste0("Accuracy_mean: " , score_mean, "\n"), file = output_file, append = TRUE)
print(x = list.files(path = output, all.files = TRUE, full.names = TRUE, recursive = TRUE) )
