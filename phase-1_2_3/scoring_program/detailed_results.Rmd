...
# if (!exists("method")) {method = "nnls"}
# if (!exists("type")) {type = "1"}
# source("datasets.R")
library(ggplot2)

# datasets_long = datasets
dic_datasets2short <- list(
  invitro =  "VITR" , 
  invivo =  "VIVO" , 
  insilicopseudobulk=  "SBN5" , 
  insilicodirichletCopule=  "SDN5" , 
  insilicodirichletEMFAImmuneLowProp=  "SDN4" , 
  insilicodirichletEMFA=  "SDN6" , 
  insilicodirichletNoDep=  "SDE5" , 
  insilicodirichletNoDep4CTsource=  "SDEL" , 
  insilicodirichletNoDep6CTsource =  "SDC5" 
)
...
* VITR: *in vitro* dataset,
* VIVO: *in vivo* dataset where we can only evaluate prediction by correlations metric because of the ground truth,
* SBN5; pseudo bulk simulation procedure, without gene/CpG probe dependence introduce on residual error, 
* SDN5: basic simulation procedure, with proportion drawn up according to a Dirichlet law and without gene/CpG probe dependence introduce on residual error, 
* SDN4: basic simulation procedure with one less cell type than reference, with proportion drawn up according to a Dirichlet law and without gene/CpG probe dependence introduce on residual error,
* SDN6: basic simulation procedure with one more cell type than reference, with proportion drawn up according to a Dirichlet law and without gene/CpG probe dependence introduce on residual error,
* SDE5: EMFA simulation procedure, with proportion drawn up according to a Dirichlet law and with gene/CpG probe dependence introduce on residual error by a factor model, 
* SDEL: EMFA simulation procedure with low proportion to the immune cell type (more to the normal one), with proportion drawn up according to a Dirichlet law and with gene/CpG probe dependence introduce on residual error by a factor model, 
* SDC5: Copula simulation procedure, with proportion drawn up according to a Dirichlet law and with gene/CpG probe dependence introduce on residual error by an empirical copula.
...
eval_Aitchison = function(A_real, A_pred, min = 1e-9) {
  ...
  return(coda.base::dist(A_real, A_pred, method = "aitchison")[1]) # do not use rbind, is cause trouble
}
  ...
  args = lapply(seq_along(datasets), function(data) {
    data.frame(
      ...
      "Aitchison" = sapply(seq(ncol(ground_truth_corrected[[data]])), function(sample) {
        eval_Aitchison(ground_truth_corrected[[data]][,sample], prediction_corrected[[data]][,sample])
      }),
...
boxplots_metric = ggplot(df_metric, aes(x = Dataset, y = metric_score, fill = Dataset)) +
  ...
  geom_boxplot(position=position_dodge2(padding = 0.2)) +
  labs(
    title = paste("Metrics scores per sample among datasets by your method"),
    x = "Datasets",
    y = "Scores"
  ) +
  theme_light() +
...
scatter_prop = ggplot(df_scatter) +
  ...
  labs(
      ...
      y = "Prediction",
      title = paste("Scatter plot of predicted and true proportion by your method among all datasets")
    ) +
    scale_x_continuous(
...
for (data in seq_along(datasets)) {
  ...
  fmsb::radarchart(
    ...
    title = paste("Global metrics scores by your method\non dataset", datasets[data]),
    #custom polygon
  ...
  # Comment HB: the visualization by a radar/spider plot is intrinsically linked to the aggregate score for defining the scope (min and max) for each variable.
}
par(mfrow = c(1,1))
```
