...
def program(mix=None, ref=None, **kwargs):
  ...
  # the reference file contains a list of 3 elements, among which is the list of sc references (one per dataset)
  # for each sc dataset, we have 2 elements: a sparse matrix with the counts, and a list of metadata
  # the metadata you're interested in is the vector of cell types
  #  ref_scRNA contains 
  #  ref_scRNA$ref_sc_peng$counts                
  #  ref_scRNA$ref_sc_peng$metadata$cell_type    
  #  ref_scRNA$ref_sc_peng$metadata$sample       
  #  ref_scRNA$ref_sc_baron$counts               
  #  ref_scRNA$ref_sc_baron$metadata$cell_type   
  #  ref_scRNA$ref_sc_baron$metadata$sample      
  #  ref_scRNA$ref_sc_raghavan$counts            
  #  ref_scRNA$ref_sc_raghavan$metadata$cell_type
  #  ref_scRNA$ref_sc_raghavan$metadata$sample
  # In this script we can access all data inside ref_scRNA using index. For the count matrix though, see below.  
  # For instance to access ref_scRNA$ref_sc_peng$metadata$sample you have to type ref_scRNA[0][1][1] 
  # The counts are accessible with the python function below : read_data_python.get_ref_counts_df


  from attachement import read_data_python
  (df_counts_peng,df_counts__baron,df_counts_raghavan) = read_data_python.get_ref_counts_df()
  print(df_counts__baron.shape)
